...
let ComplaintMapping = require("./db/complaint-mapping");
...
app.post("/complain", (request, response) => {
...
app.post("/assign", (request, response) => {
  const newComplaintMapping = new ComplaintMapping({
    complaintID: request.body.complaintID,
    engineerName: request.body.engineerName,
  });
  newComplaintMapping
    .save()
    .then((result) => {
      response.status(201).send({
        message: "Complain map Successfully",
        result,
      });
    })
    .catch((error) => {
      response.status(500).send({
        message: "Error creating complain-mapping",
        error,
      });
    });
});

app.get("/engineer", (request, response) => {
  User.getEngineer((err, engineer) => {
    if (err) {
      response.status(500).json({ error: err.message });
    } else {
      response.json(engineer);
    }
  });
});
...
app.get("/complains", (request, response) => {
  Complaint.find({})
    ...
    .catch((error) => {
      response.status(500).send({
        message: "Error retrieving complains",
        error,
      });
    });
});
...
app.get("/jeng", auth, (request, response) => {
  response.send({ message: "You are authorized to access me" });
});
module.exports = app;
